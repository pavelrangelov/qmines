...
#define PAUSED_TEXT_MARGIN 10
		...
		painter.drawText(
				QRect(m_PausedX + PAUSED_TEXT_MARGIN, m_PausedY + PAUSED_TEXT_MARGIN, m_PausedW - 2 * PAUSED_TEXT_MARGIN, m_PausedH - 2 * PAUSED_TEXT_MARGIN),
...
void MainFrame::mousePressEvent(QMouseEvent *me) {
	...
	if ((pos.x() <= m_FieldBegin) || (pos.x() >= m_FieldBegin + m_SquareWidth * m_NumX) || (pos.y() <= 0) || (pos.y() >= m_SquareWidth * m_NumY)) {
		return;
	}
	...
	for (x = 0; x < m_NumX; x++) {
		for (y = 0; y < m_NumY; y++) {
			...
			if ( _NW(x,y,maxx,maxy) && HAS_MINE(x - 1, y - 1))
				value++;
			if ( _N (x,y,maxx,maxy) && HAS_MINE(x + 0, y - 1))
				value++;
			if ( _NE(x,y,maxx,maxy) && HAS_MINE(x + 1, y - 1))
				value++;
			if ( _W (x,y,maxx,maxy) && HAS_MINE(x - 1, y + 0))
				value++;
			if ( _E (x,y,maxx,maxy) && HAS_MINE(x + 1, y + 0))
				value++;
			if ( _SW(x,y,maxx,maxy) && HAS_MINE(x - 1, y + 1))
				value++;
			if ( _S (x,y,maxx,maxy) && HAS_MINE(x + 0, y + 1))
				value++;
